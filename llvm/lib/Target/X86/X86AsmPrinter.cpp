...
void X86AsmPrinter::emitFunctionBodyEnd() {
  ...
  // bw1-decomp: Emit trailing bytes after the function body (after ret).
  // These are unreachable junk bytes (NOPs, jump table stubs, etc.) that
  // exist in the original binary but cannot be generated by C code.
  if (MF->getFunction().hasFnAttribute("trailing_bytes")) {
    StringRef Bytes =
        MF->getFunction()
            .getFnAttribute("trailing_bytes")
            .getValueAsString();
