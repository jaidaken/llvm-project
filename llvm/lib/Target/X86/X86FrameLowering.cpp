...
void X86FrameLowering::emitEpilogue(MachineFunction &MF,
                                    MachineBasicBlock &MBB) const {
  // bw1-decomp: NoRet suppresses the epilogue AND erases the ret instruction
  // generated by ISel. The inline asm provides its own terminator (jmp, ret, etc.).
  if (MF.getFunction().hasFnAttribute(Attribute::NoRet)) {
    // Erase any RET/RETL terminator from the MBB.
    for (auto I = MBB.end(), E = MBB.begin(); I != E;) {
      --I;
      if (I->isReturn()) {
        I->eraseFromParent();
        break;
